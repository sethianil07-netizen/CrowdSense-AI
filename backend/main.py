...
async def start_simulation(request: SimulationStartRequest):
    """Start (or restart) the default REST-tracked simulation for a given
    venue. For real-time streaming, clients should instead connect over
    Socket.io and emit 'start_simulation' (see sockets.py)."""
    try:
        venue_data = _find_venue(request.venue_id)
        venue = Venue.load_from_json(venue_data)
        rest_manager.init_simulation(venue, num_agents=request.num_agents)
        await rest_manager.start()
        return SimulationStartResponse(
            status="started", venue_id=venue.venue_id, num_agents=request.num_agents,
        )
    except HTTPException:
        raise
    except Exception as exc:  # noqa: BLE001
...
@app.post("/api/simulation/stop", response_model=SimulationStopResponse)
...
async def simulation_state():
    """Poll the current state of the REST-tracked simulation (useful for
    clients that don't want to use Socket.io)."""
    if not rest_manager.venue:
        raise HTTPException(status_code=400, detail="No simulation has been started yet")
...
@app.get("/api/health")
async def health():
    return {"status": "ok", "service": "crowdsense-backend"}
